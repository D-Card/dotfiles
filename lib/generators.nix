{
  ...
  */
  mkOverlays = overlaysDir:
    ...
    (lib.personal.rakeLeaves overlaysDir);
  ...
  */
  mkStaticConfigs = hosts: targets: extraLabels:
    lib.mapAttrsToList (_: {config, ...}: {
      ...
      labels = (lib.filterAttrs (_: v: v != null) config.personal.labels) // (builtins.listToAttrs (builtins.map (label: label config) extraLabels));
    })
    hosts;
in {
